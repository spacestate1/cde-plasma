...
// arrow buttons). The previous inset narrowed the thumb channel.
constexpr int kScrollCrossInset = 0;
// Use the same bevel thickness for both the arrow buttons and the thumb so
// the whole scrollbar reads as a straight, square control.
                                 ...
                                 const QWidget *widget) const
{
    if (type == CT_MenuItem) {
        ...
        const int shortcutWidth = menuItem->reservedShortcutWidth > 0 ? menuItem->reservedShortcutWidth + kMenuShortcutGap : 0;
                                  ...
                                  const QWidget *widget) const
{
    ...
    if (rects.slider.isValid()) {
        drawBevel(painter, rects.slider, sliderPressed ? pressedFill : buttonFill, slider->palette, sliderPressed, kScrollBevelThickness);
    }
}
